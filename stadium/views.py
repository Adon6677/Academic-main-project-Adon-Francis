...
def register(request,pid=None):
    ...
    if pid:
        ...
        profile = Stadium_Profile.objects.get(user=user)
...
def Single_Profile(request):  #get single data 
    single_data = Stadium_Profile.objects.get(user=request.user)
    dict = {
        'data':single_data
    }
...
def All_Profile(request):  # getmultiple data
    single_data = Stadium_Profile.objects.all()
    dict = {
        'data':single_data
    }
...
def delete_user(request,pid):
    profile = Stadium_Profile.objects.get(id = pid)
    profile.delete()
...
def delete_category(request,pid):
    game = GameCategory.objects.get(id=pid)
    game.delete()
    messages.success(request, "Deleted successfully")
...
def add_game_category(request, pid=None):
    game_cat = None
    if pid:
        game_cat = GameCategory.objects.get(id=pid)
    if request.method == "POST":
...
def view_game_category(request):
    v_game = GameCategory.objects.all()
